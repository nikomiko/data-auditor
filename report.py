...
def _add_pivot_table(xlsx_bytes: bytes, col_names: list, n_rows: int) -> bytes:
    ...
    Crée un onglet "ANALYSE" avec une pivot table dynamique :
      - Source  : table DATA de l'onglet DATA
      - Lignes  : Type (externe) > Règle (interne)
      - Valeurs : Nombre d'occurrences (Count de Clé)

    openpyxl ne sait pas écrire les pivot tables ; on injecte directement le XML
    OOXML dans le ZIP. La pivot table se rafraîchit automatiquement à l'ouverture
    (refreshOnLoad="1") depuis la table DATA du même classeur.
    """
    ...
    NS_PKG = 'xmlns="http://schemas.openxmlformats.org/package/2006/relationships"'

    REL_WS   = 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet'
    REL_PC   = 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/pivotCacheDefinition'
    REL_PCR  = 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/pivotCacheRecords'
    REL_PT   = 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/pivotTable'

    CT_WS    = 'application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml'
    CT_PC    = 'application/vnd.openxmlformats-officedocument.spreadsheetml.pivotCacheDefinition+xml'
    CT_PCR   = 'application/vnd.openxmlformats-officedocument.spreadsheetml.pivotCacheRecords+xml'
    ...
    cache_fields = ''.join(
        f'<cacheField name={_qa(c)} numFmtId="0"><sharedItems/></cacheField>'
        for c in col_names
    )
    cache_def = (
    ...
    # ── pivotCacheRecords (vide — rechargé depuis la source) ──
    cache_records = (
        '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
        f'<pivotCacheRecords {NS_SS} count="0"/>'
    )

    # ── Rels cache → records ──────────────────────────────────
    cache_def_rels = (
        '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
        f'<Relationships {NS_PKG}>'
        f'<Relationship Id="rId1" Type="{REL_PCR}" Target="pivotCacheRecords1.xml"/>'
        '</Relationships>'
    )

    # ── pivotTableDefinition ──────────────────────────────────
    # Lignes : champ 1 (Type) externe, champ 2 (Règle) interne
    # Valeurs : Count de champ 0 (Clé)
    pf_parts = []
    for i in range(n):
        if i in (1, 2):   # Type, Règle → axe ligne, outline
            pf_parts.append('<pivotField axis="axisRow" compact="0" showAll="0" outline="1"/>')
        else:
            pf_parts.append('<pivotField compact="0" showAll="0"/>')

    pivot_tbl = (
        '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
        f'<pivotTableDefinition {NS_SS}'
        ' name="TCD_Audit" cacheId="1"'
        ' dataCaption="Valeurs" grandTotalCaption="Total g\u00e9n\u00e9ral"'
        ' rowGrandTotals="1" colGrandTotals="0"'
        ' compact="0" compactData="0" outline="1" outlineData="1">'
        '<location ref="A1" firstHeaderRow="1" firstDataRow="2" firstDataCol="1"/>'
        f'<pivotFields count="{n}">{"".join(pf_parts)}</pivotFields>'
        '<rowFields count="2"><field x="1"/><field x="2"/></rowFields>'
        '<colFields count="1"><field x="-2"/></colFields>'
        '<dataFields count="1">'
        '<dataField name="Nb occurrences" fld="0" subtotal="count"/>'
        '</dataFields>'
    ...
    # ── Rels pivot table → cache ──────────────────────────────
    pivot_tbl_rels = (
        '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
        f'<Relationships {NS_PKG}>'
        f'<Relationship Id="rId1" Type="{REL_PC}"'
        ' Target="../pivotCache/pivotCacheDefinition1.xml"/>'
        '</Relationships>'
    )

    # ── Feuille ANALYSE (hôte de la pivot table) ──────────────
    pivot_sheet = (
        '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
        f'<worksheet {NS_SS} {NS_R}>'
        '<sheetViews><sheetView workbookViewId="0"/></sheetViews>'
        '<sheetData/>'
        '</worksheet>'
    )

    # ── Rels feuille → pivot table ────────────────────────────
    pivot_sheet_rels = (
        '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
        f'<Relationships {NS_PKG}>'
        f'<Relationship Id="rId1" Type="{REL_PT}"'
        ' Target="../pivotTables/pivotTable1.xml"/>'
        '</Relationships>'
    )

    # ── Helpers : modifier les fichiers ZIP existants ─────────

    def _patch_content_types(data: bytes) -> bytes:
        additions = (
            f'<Override PartName="/xl/worksheets/sheet2.xml" ContentType="{CT_WS}"/>'
            f'<Override PartName="/xl/pivotCache/pivotCacheDefinition1.xml" ContentType="{CT_PC}"/>'
            f'<Override PartName="/xl/pivotCache/pivotCacheRecords1.xml" ContentType="{CT_PCR}"/>'
            f'<Override PartName="/xl/pivotTables/pivotTable1.xml" ContentType="{CT_PT}"/>'
        )
        s = data.decode('utf-8')
        return s.replace('</Types>', additions + '</Types>').encode('utf-8')

    def _patch_workbook(data: bytes) -> bytes:
        import re as _re
        REL = 'http://schemas.openxmlformats.org/officeDocument/2006/relationships'
        NS  = f'xmlns:r="{REL}"'   # déclaré sur chaque élément inséré pour garantir la portée
        s = data.decode('utf-8')
        s = _re.sub(
            r'(</sheets>)',
            f'<sheet {NS} name="ANALYSE" sheetId="2" r:id="rId200"/>' + r'\1',
            s, count=1,
        )
        s = s.replace(
            '</workbook>',
            f'<pivotCaches><pivotCache {NS} cacheId="1" r:id="rId201"/></pivotCaches></workbook>',
        )
        return s.encode('utf-8')

    def _patch_workbook_rels(data: bytes) -> bytes:
        additions = (
            f'<Relationship Id="rId200" Type="{REL_WS}" Target="worksheets/sheet2.xml"/>'
            f'<Relationship Id="rId201" Type="{REL_PC}" Target="pivotCache/pivotCacheDefinition1.xml"/>'
        )
        s = data.decode('utf-8')
        return s.replace('</Relationships>', additions + '</Relationships>').encode('utf-8')

    # ── Reconstruction du ZIP ─────────────────────────────────
    src = io.BytesIO(xlsx_bytes)
    dst = io.BytesIO()

    with _zf.ZipFile(src, 'r') as zin, _zf.ZipFile(dst, 'w', _zf.ZIP_DEFLATED) as zout:
        for info in zin.infolist():
            raw = zin.read(info.filename)
            if info.filename == '[Content_Types].xml':
                raw = _patch_content_types(raw)
            elif info.filename == 'xl/workbook.xml':
                raw = _patch_workbook(raw)
            elif info.filename == 'xl/_rels/workbook.xml.rels':
                raw = _patch_workbook_rels(raw)
            zout.writestr(info.filename, raw)

        zout.writestr('xl/pivotCache/pivotCacheDefinition1.xml',       cache_def.encode('utf-8'))
        zout.writestr('xl/pivotCache/pivotCacheRecords1.xml',           cache_records.encode('utf-8'))
        zout.writestr('xl/pivotCache/_rels/pivotCacheDefinition1.xml.rels', cache_def_rels.encode('utf-8'))
        zout.writestr('xl/pivotTables/pivotTable1.xml',                 pivot_tbl.encode('utf-8'))
        zout.writestr('xl/pivotTables/_rels/pivotTable1.xml.rels',      pivot_tbl_rels.encode('utf-8'))
        zout.writestr('xl/worksheets/sheet2.xml',                       pivot_sheet.encode('utf-8'))
        zout.writestr('xl/worksheets/_rels/sheet2.xml.rels',            pivot_sheet_rels.encode('utf-8'))
